...
var bodyParser = require('body-parser') //Body'den bir data alacaksak express ile body-parser'ı kütüphanemize implement etmeliyiz.
...
app.get('/api/products', function(req, res){
    ...
    if (orderBy == undefined){
        ...
        products = _.orderBy(productsData, ['name'], ['desc']);
        res.json(products)
    }
...
//Bu endpoint tüm ürünleri getirir
app.get('/api/products', function(req, res){
    res.json(productsData);
})

//Bu endpoint dışarıdan aldığı id'ye ait tüm ürünleri getirir
app.get('/api/products/:id', function(req,res){
    
    //Gelen requestin parametresinden id'yi al 
    var productId = req.params.id;

    //Bu id'ye ait ürünü bul.
    var product = productsData.find(q => q.id == productId);
    if (product != null && product != undefined) {
        //Bulduğu ürünü fırlattım
        res.json(product)
    }
    else{
...
//Bu endpoint dışarıdan aldığı categoryId'ye ait ürünleri getirir
app.get('/api/products/category/:id', function(req,res){
    let categoryId = req.params.id
    let filteredProducts = productsData.filter(q => q.categoryId == categoryId);

    res.json(filteredProducts)
})

//Yeni bir ürün ekleme (). Burada serverSide validation işlemi yaptık.
app.post('/api/products',

    body('name').notEmpty(),
    body('unitPrice').notEmpty().withMessage("unitPrice alanı boş geçilemez!").isNumeric().withMessage("unitPrice numeric olmalıdır!")
    , function (req, res) {
        ...
        //son elementi buluyoruz ve id'sine 1 ekliyoruz. DB bizim olmadığı için mecburuz.
    ...
    res.status(201).json(newProduct) //bu satırdan sonra kod blokları çalışmaz, çünkü cevebı zaten dönmüş olur
...
app.delete('/api/products/:id', function(req, res) {
    ...
    console.log(`Before length: ${productsData.length}`.red) //template literal style, bir üst satırın farklı bir yazılış tarzı.
    //console.log('Before...'.red)
    //console.log('BeforeLength',productsData.length);
    productsData = productsData.filter(q => q.id != productId);
    //console.log('AfterLength',productsData.length);
    console.log(("Before length: " + productsData.length + "").blue);
